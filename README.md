...
The raw columns are populated as follows:

- `time`: timestamp generated by the Raspberry Pi when the sample is logged;
- `tag`: configured human-readable alias, such as `dw03`;
- `a16`: short tag identifier reported by the node;
- `seq`: ranging sequence number from firmware field `R`;
- `range_cm`: tag-node distance in centimetres from firmware field `D`;
- `pdoa_deg`: raw PDoA value in degrees from firmware field `P`;
- `x_cm`, `y_cm`: node-estimated tag coordinates in centimetres from `Xcm` and `Ycm`;
- `clk_ppm`: clock offset in ppm, converted from firmware field `O` (hundredths of ppm);
- `t_us`: node SuperFrame timestamp in microseconds from firmware field `T`.

The normal TWR payload also contains `V` and the tag accelerometer values
`X`, `Y`, and `Z`, but these fields are currently not written to the CSV.
The optional debug payload may contain `Tm` and `Ts`, the master and slave
temperatures in degrees Celsius; those fields are not part of the normal
payload and are not currently logged.
